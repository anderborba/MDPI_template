...
\Title{Fast Spatial Denoising of InSAR Interferograms via Empirical Statistical Modeling}

% MDPI internal command: Title for citation in the left column
\TitleCitation{Fast Spatial Denoising of InSAR Interferograms via Empirical Statistical Modeling}
...
\abstract{
SAR interferometry (InSAR) provides a framework for extracting high-resolution topographic information and detecting surface deformation. 
...
%
This work addresses the challenge of filtering the unwrapped phase, a process traditionally reliant on accurate coherence images to identify reliable pixels.
%
We evaluate three statistically-based spatial filters for phase noise reduction. 
The Enhanced Lee filter, which utilizes spatial adaptation and a physically-grounded probability model, serves as the baseline for comparison. 
We examine the Gierull model, which improves computational efficiency by restricting the parameter space. To further reduce execution time, we propose and evaluate two empirical alternatives: the truncated wrapped normal (TcN) and the truncated wrapped Cauchy (TcC) distributions. 
Results indicate that these empirical models significantly reduce computational demand without degrading the quality of the filtered phase.
%
...
\subsection{Simulated Data}\label{subsec:materials_sim_img}
...
The InSAR images used to test the filters were obtained from~\url{http://uavsar.jpl.nasa.gov/}. 
We selected three examples described below:
La Cumbre Volcano,
Los Alamos, and
Robledo Volcano.
...
The Region of Interest (ROI) of the InSAR image presented in Figure~\ref{fig:insar_la_cumbre_InSAR} has $346\times492$ pixels.
...
\subsection{Statistical Models and Filters}\label{subsec:methods}
...
\begin{figure}[hbt]
	\centering
	\includegraphics[width=1.0\linewidth]{std_dev_vs_coherence_Gierull}
...
We used a Sentinel-1 SAR image (C-band) and a UAVSAR image (L-band). A simulated image was used to test the proposed approaches and compare them with well-established methods. First, we analyzed the simulated data, as described in Ref.~\cite{InSAR-MONet2022}. 
Thereafter, we analyzed the InSAR images from La Cumbre volcano, Los Alamos, and Robledo Volcano.
...
To analyze the results more consistently, the Kolmogorov-Smirnov (KS) test was applied to two filtered images, comparing TcNfilter with LInSARRFE and TcCfilter with LInSARRFE. We used the test to obtain a statistically significant measure of the phase difference between the filtered images, rather than the image-level measures used previously.

The test assesses the null hypothesis $H_0$, which states that the two samples are drawn from the same distribution. 
The results obtained were $\text{KS} = 0.1210$ for TcNfilter 
and $\text{KS} = 0.1134$ for TcCfilter, both leading to 
$p{\text{-value}}<10^{-6}$ and indicating the rejection of $H_0$ any practical significance level. 
However, the maximum absolute difference between the empirical CDFs and the physical CDF based on the Gierull model is approximately \qty{12}{\percent}. 
Altogether, the results of the hypothesis tests, combined with the analysis of the RMSE and SSIM metrics, suggest that while the empirical filters do not exactly reproduce the phase distribution of the LInSARRFE, these distributional differences are modest. 
Furthermore, they do not result in a significant degradation of filtering quality under the tested conditions.

Regarding SSIM, the metric that evaluates the structural similarity between the filtered interferogram and the reference image (noise-free), the Refined Lee filter shows a slight advantage, with a value closer to $1$. 
...
In summary, while the TcN filter is the most computationally efficient, we suggest the TcC filter as the preferred practical choice due to its superior denoising capability at a negligible additional cost. 
For applications necessitating a physically-based approach, the Gierull model serves as a high-performance alternative to the Enhanced Lee filter, yielding equivalent results for integer looks but with vastly improved execution speed.
