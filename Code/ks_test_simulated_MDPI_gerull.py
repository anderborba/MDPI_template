"""
Kolmogorov-Smirnov test: empirical filter distributions vs Gierill Reference
Simulated InSAR data – MDPI Remote Sensing

Filters compared:  TcNfilter, TcCfilter
Reference:         LInSARRFE.tif  
...
ref   = phases["LInSARRFE"]
names = ["TcNfilter", "TcCfilter"]
# ── KS tests ──────────────────────────────────────────────────
print("\n" + "="*60)
...
for name in names:
    ...
    if p > 0.05:
        interp = (f"the distribution of phases filtered by {name} is "
                  f"statistically indistinguishable from the LInSARRFE "
        ...
        interp = (f"the distribution of phases filtered by {name} differs "
                  f"significantly from the LInSARRFE "
...
ax.set_title("ECDF comparison: empirical filters vs LInSARRFE", fontsize=10)
