...
\isAPAStyle{%
	...
	\isChicagoStyle{%
		\AuthorCitation{Andson A. de Borba, Araújo, Joselito Elias, and Alejandro C. Frery.}%
	}{%
		\AuthorCitation{Borba, A. A.; Araújo, J. E.; Frery, A. C.}%
	}%
}
...
%
However, the noise inherent in interferometric images
...
confirm this affirmation, we use four types of test images, including a simulated dataset for
...
%
The study found a bottleneck in the computational cost of numerical integration that the empirical
models impose on the filtering, a topic that requires further investigation.
%
In this article,
the authors propose three new techniques that demonstrate great computational efficiency
...
\begin{itemize}[label=$\bullet$]
	\item We quantified the difference between two physical models (a complete model and a mode with restricted parameter space) for InSAR data.
    \item We proposed two empirical circular models for noise reduction.
    \item We show that these empirical models produce as good results as the physical one with a significant reduction in computational processing time.
\end{itemize}
%
...
To address the need for fast and effective interferometric phase‑filtering techniques, we propose three new filters: LInSARRFE, TcNfilter, and TcCfilter.
LInSARRFE is derived from a physically based probability distribution that models SAR signal formation and its representation as a phase image.
TcNfilter and TcCfilter, in contrast, rely on empirical probability distributions designed to reduce computational complexity.
These results provide efficient and robust alternatives for InSAR data processing, especially in applications requiring high‑speed analysis.
...
In this work, we quantify the differences between Lee et al's and Gierull's versions of the same physical model, and introduce two empirical distributions designed to further reduce computational cost and numerical stability.
These models are incorporated into an adaptive filtering framework that adjusts to local phase variability.
We used the refined Lee filter as a reference, as it represents the state of the art in InSAR data filtering. 
...
We analyzed data from the eruption of the La Cumbre volcano on Fernandina Island in the Galápagos archipelago, Ecuador, on January 12, 2020, see~\cite{pechnikovLaCumbre}. 
The Region of Interest (ROI) presented in Figure~\ref{fig:insar_la_cumbre:c} has $346\times492$ pixels.

\begin{figure}[hbt]
\centering
\includegraphics[width=0.567\linewidth]{interferograma_LaCumbe}
\caption{La Cumbre Volcano InSAR Image}
\label{fig:insar_la_cumbre:c}
\end{figure}

\subsubsection{Los Alamos InSAR Image}\label{subsec:materials_los_alamos}

Figure~\ref{fig:insar_uavsar_los_alamos:a} shows the InSAR image of Los Alamos, USA, to the left, while the right image is the region of interest (ROI) that we use for testing.
...
Figure~\ref{fig:insar_uavsar_robledo:b} shows the InSAR image of Robledo Volcano, Argentina, to the left, while the ROI is shown to the right.
...
%       \includegraphics[width=0.51\linewidth]{arquivo_fix_robledo.pdf}
    ...
    \caption{Interferometric phase images acquired by the UAVSAR sensor: (a) Los Alamos, USA; (b) Robledo Volcano, Argentina.}
     \label{fig:insar_uavsar} 
\end{figure}

Table~\ref{tab:uavsar_charac} presents the main image acquisition characteristics of these InSAR images. 

\begin{table}[hbt]
	\caption{UAVSAR Image Characteristics.\label{tab:uavsar_charac}}
	\begin{tabularx}{\textwidth}{
		...
		Date of Acquisition   & 12-Jan-2020           &  26-Feb-2020                   & 24-Mar-2013  \\
		Dimension             & $972\times 920$       & $6315\times 4631$              & $14702\times 24680$  \\
		ROI                   & $346\times492$        & $500\times 500$                & $500\times 500$  \\
		(Lat, Lon)            & (\qty{-91.66}{\degree}, \qty{-0.50}{\degree})       & (\qty{36.03}{\degree}, \qty{-106.4}{\degree})                & (\qty{-26.6}{\degree}, \qty{-67.52}{\degree})\\
		Bandwidth             & \qty{80}{\mega\hertz} &  \qty{80}{\mega\hertz}         & \qty{80}{\mega\hertz}\\
		Center Wavelength     &  --                   & \qty{23.8403545}{\centi\metre} &\qty{23.8403545}{\centi\metre} \\
...
We used a Sentinel-1 SAR image (band C) and a UAVSAR image. A simulated image was used to test the proposed approaches and compare them with well-established methods. First, we analyzed the simulated data, as described in Ref.~\cite{InSAR-MONet2022}. Thereafter, we analyzed the InSAR images from La Cumbre volcano, Los Alamos, and Robledo Volcano.
...
As shown in Table~\ref{TabEficiencia_simu}, the TcCfilter exhibits the lowest RMSE and the highest accuracy among the analyzed filters. Additionally, the proposed filters LInSARRFE and TcNfilter present RMSE values that are only slightly higher than those of the Refined Lee filter.

\begin{table}[hbt] 
	\caption{Efficiency of filters for simulated data.\label{TabEficiencia_simu}}
	\begin{tabularx}{\textwidth}{
			l % Filter name (left-aligned)
			>{\centering\arraybackslash}X  % RMSE
			>{\centering\arraybackslash}X  % SSIM
			>{\centering\arraybackslash}X  % RT (s)
			>{\centering\arraybackslash}X  % Time Saved
		}
		\toprule
		\textbf{Filter} & \textbf{RMSE} & \textbf{SSIM} & \textbf{RT (s)} & \textbf{Time Saved (\%)}\\
		\midrule
		Ref. Lee     & 1.255 & \textbf{0.687} & 4790.76 & --- \\
		LInSARRFE    & 1.269 & 0.644          & 1460.44 & 69.51 \\
		TcNfilter    & 1.297 & 0.670          & \textbf{404.10}  & 91.56 \\
		TcCfilter    & \textbf{1.251} & 0.641 & 427.73  & 91.07 \\
		\bottomrule
	\end{tabularx}
\end{table}
...
Therefore, the proposed filters excel over the Refined Lee filter in terms of accuracy, structural preservation, and processing efficiency.
...
TcCfilter achieved the lowest RMSE values in simulated data, indicating high precision in phase estimation. 
...
The Refined Lee InSAR filter was chosen as the sole benchmark because it’s the basic spatial-domain statistical filtering method from which both the Enhanced Refined Lee variant and the methods in this work are derived. A wider comparison of all Lee filter variants is beyond the scope of this study and is a potential future investigation.
...
The proposed filters’ computational efficiency was crucial in the evaluation. 
...
These characteristics are promising given the growing demand for efficient algorithms in missions like NISAR, which generates massive amounts of interferometric data.
...
This idea can improve numerical integration and parallel computing.
