...
%DIF DEL R0-FastNoiseInterferometricReduction.tex   Wed Apr  8 23:07:17 2026
%DIF ADD R1-FastNoiseInterferometricReduction.tex   Sun Apr 12 15:35:27 2026
...
\Title{Fast Noise Reduction in InSAR Data with Empirical Statistical Models}

% MDPI internal command: Title for citation in the left column
\TitleCitation{Fast Noise Reduction in InSAR Data with Empirical Statistical Models}
...
\isAPAStyle{%
	...
	\isChicagoStyle{%
%DIF 129c129
%DIF < 		\AuthorCitation{Andson A. de Borba, Araújo, Joselito Elias, and Alejandro C. Frery.}%
%DIF -------
		\AuthorCitation{Anderson A. de Borba, Araújo, Joselito Elias, and Alejandro C. Frery.}% %DIF > 
%DIF -------
	}{%
		\AuthorCitation{Borba, A. A.; Araújo, J. E.; Frery, A. C.}%
	}%
...
\abstract{
SAR interferometry images (InSAR) enable the extraction of detailed information from
Earth’s surface, including high-resolution topographic maps and precise deformation
...
% Keywords
\keyword{InSAR; phase noise filtering; empirical and physical statistical models; coherence estimation; computational efficiency} 
...
An interferogram is an image in which each pixel encodes the phase difference between the corresponding pixels of two co-registered complex SAR images obtained from different viewpoints.
...
The enormous quantity of forthcoming InSAR data highlights the urgent need for filters that are not only accurate but also computationally efficient. 
...
\DIFaddend These results provide efficient and robust alternatives for InSAR data processing, especially in applications requiring high‑speed analysis.

Existing InSAR filtering techniques can be broadly categorized into three groups:
Spatial‑domain methods, such as Boxcar, median, mean, Lee InSAR, and Refined Lee InSAR;
Transform‑domain methods, such as the Goldstein filter; and
Non‑local approaches that exploit image redundancy~\cite{ANonlocalInSARFilterforHighResolutionDEMGenerationfromTanDEMXInterferograms}.
Among these three groups, our research focuses on spatial-domain filters grounded in statistical modeling. 

Lee et al.~\cite{IntensityAndPhaseStatisticsOfMultilookPolarimetricAndInterferometricSARimagery} introduced a physically based phase distribution, although it is computationally demanding and prone to numerical instability. 
Gierull~\cite{ClosedFormExpressionsforInSARSampleStatisticsandItsApplicationtoNonGaussianData} mitigated these issues by restricting the parameter space, improving both stability and processing time. 
Furthermore, recent work continues to explore efficient alternatives to these classical models~\citep{AngularConsistencyAdaptiveFilteringandComplexGeneralizedGaussianBasedPhaseLinkingforDSInSAR,CLTCPLaRobustMathematicalFrameworkandAlgorithmforInSARPhaseLinkingUsingtheCentralLimitTheoremofCircularStatistics}.
...
\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.567\linewidth]{interferograma_LaCumbe}
%DIFDELCMD < %%%
...
%DIFDELCMD < 		\includegraphics[width=1.0\linewidth]{los_alamos_orig_and_crop_500}
%DIFDELCMD <      }\\
...
%DIFDELCMD <        \includegraphics[width=1.0\linewidth]{robledo_orig_and_crop_500}
%DIFDELCMD <      }
%DIFDELCMD <     %%%
...
%DIF >        \includegraphics[width=0.51\linewidth]{arquivo_fix_robledo.pdf}
       \DIFaddbeginFL \includegraphics[width=1.0\linewidth]{robledo_orig_and_crop_500}
     %DIF > }
...
\begin{table}[hbt]
	\caption{\DIFdelbeginFL \DIFdelFL{UAVSAR }\DIFdelendFL Image Characteristics.\label{tab:uavsar_charac}}
	\begin{tabularx}{\textwidth}{
		...
		\midrule
		\DIFaddbeginFL \DIFaddFL{Sensor                }&   \DIFaddFL{Sentinel-1          }& \DIFaddFL{UAVSAR                         }& \DIFaddFL{UAVSAR}\\
		\DIFaddFL{Band                  }&  \DIFaddFL{C--band              }& \DIFaddFL{L--band                        }& \DIFaddFL{L--band }\\
		\DIFaddendFL Resolution            & \qty{15}{\metre}      & \qty{6}{\metre}                & \qty{6}{\metre} \\
		Date of Acquisition   & 12-Jan-2020           &  \DIFaddbeginFL \DIFaddFL{19 and }\DIFaddendFL 26-Feb-2020                   & 24-Mar-2013  \\
		Dimension             & $972\times 920$       & $6315\times 4631$              & $14702\times 24680$  \\
		ROI                   & $346\times492$        & $500\times 500$                & $500\times 500$  \\
		(Lat, Lon)            & (\DIFdelbeginFL \DIFdelFL{\mbox{%DIFAUXCMD
\qty{-91.66}{\degree}}\hskip0pt%DIFAUXCMD
, \mbox{%DIFAUXCMD
\qty{-0.50}{\degree}}\hskip0pt%DIFAUXCMD
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\qty{-91.53}{\degree}}\hskip0pt%DIFAUXCMD
, \mbox{%DIFAUXCMD
\qty{-0.37}{\degree}}\hskip0pt%DIFAUXCMD
}\DIFaddendFL )       & (\qty{36.03}{\degree}, \qty{-106.4}{\degree})                & (\qty{-26.6}{\degree}, \qty{-67.52}{\degree})\\
		Bandwidth             & \qty{80}{\mega\hertz} &  \qty{80}{\mega\hertz}         & \qty{80}{\mega\hertz}\\
		\DIFdelbeginFL \DIFdelFL{Center }\DIFdelendFL Wavelength            &\DIFdelbeginFL \DIFdelFL{--                   }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\qty{5.6}{\centi\metre} }\hskip0pt%DIFAUXCMD
}\DIFaddendFL & \qty{23.8403545}{\centi\metre} &\qty{23.8403545}{\centi\metre} \\
...
%	%\begin{adjustwidth}
%	\centering
%	\subfloat[\centering\parbox{\linewidth}{Blue $L=3.49$, red $L=3$}\label{p1}]{\includegraphics[width=.45\linewidth]{Lee3.49_Gierull3}}
%	\subfloat[\centering\parbox{\linewidth}{Blue $L=10.49$, red $L=10$}\label{p2}]{\includegraphics[width=.45\linewidth]{Lee10.49_Gierull10}}
%	
%	\subfloat[\centering\parbox{\linewidth}{Blue $L=20.49$, red $L=20$}\label{p3}]{\includegraphics[width=.45\linewidth]{Lee20.49_Gierull20}}
%	\subfloat[\centering\parbox{\linewidth}{Blue $L=50.49$, red $L=50$}\label{p4}]{\includegraphics[width=.45\linewidth]{Lee50.49_Gierull50}}
%	
%	\subfloat[\centering\parbox{\linewidth}{Blue $L=100.49$, red $L=100$}\label{p5}]{\includegraphics[width=.45\linewidth]{Lee100.49_Gierull100}}
%	\subfloat[\centering\parbox{\linewidth}{Blue $L=170.49$, red $L=170$}\label{p6}]{\includegraphics[width=.45\linewidth]{Lee170.49_Gierull170}}
%	%\end{adjustwidth}
...
\end{equation}
where $F$ is the CDF of the adopted model. According to Chao et al. to~\cite{RefinedFilteringofInterferometricPhaseFromInSARData}, the value of $\psi_\xi$ can be numerically obtained from $\xi$ for all pixels. 
...
For the TcNfilter, TcCfilter, and LInSARFE filters, the phase limit $\psi_\xi$ is obtained based on the empirical models ($\mathcal{NT_C}$ and $\mathcal{CT_C}$), and on the physical model~\eqref{subsec:sec3.2.1_cap3Eq8}. 
...
We used a Sentinel-1 SAR image (\DIFdelbegin \DIFdel{band C}\DIFdelend \DIFaddbegin \DIFadd{C--band}\DIFaddend ) and a UAVSAR image \DIFaddbegin \DIFadd{(L--band)}\DIFaddend . A simulated image was used to test the proposed approaches and compare them with well-established methods. First, we analyzed the simulated data, as described in Ref.~\cite{InSAR-MONet2022}. Thereafter, we analyzed the InSAR images from La Cumbre volcano, Los Alamos, and Robledo Volcano.
...
\begin{table}[hbt]
	...
	\begin{tabularx}{\textwidth}{
		...
		\midrule
		Ref. Lee & $\theta=0$, $L=16$ & $\theta=0$, $L=16$ & $\theta=0$, $L=8$ & $\theta=0$, $L=8$ \\
...
\begin{figure}[hbt]
	%\begin{adjustwidth}{-\extralength}{0cm}
	\centering
	\subfloat[\centering\label{Original_phase}]{\includegraphics[width=.179\linewidth]{original_phase}}
	\hspace{0.01cm}
	\subfloat[\centering\label{Noise_phase}]{\includegraphics[width=.179\linewidth]{noise_phase}}
	\\
	\subfloat[\centering\label{LeeRefined}]{\includegraphics[width=.178\linewidth]{LeeRefined}}
	\hspace{0.01cm}
	\subfloat[\centering\label{LInSARRFE}]{\includegraphics[width=.179\linewidth]{LInSARRFE}}
	\hspace{0.01cm}
	\subfloat[\centering\label{TcNfilter}]{\includegraphics[width=.179\linewidth]{TcNfilter}}
	\hspace{0.01cm}
	\subfloat[\centering\label{TcCfilter}]{\includegraphics[width=.18\linewidth]{TcCfilter}}
	\\
	\includegraphics[width=.65\linewidth]{BarraCores}
	\\
	\subfloat[\centering\label{LeeRefined_unwrapped}]{\includegraphics[width=.179\linewidth]{LeeRefined_unwrapped}}
	\hspace{0.01cm}
	\subfloat[\centering\label{LInSARRFE_unwrapped}]{\includegraphics[width=.181\linewidth]{LInSARRFE_unwrapped}}
	\hspace{0.01cm}
	\subfloat[\centering\label{TcNfilter_unwrapped}]{\includegraphics[width=.18\linewidth]{TcNfilter_unwrapped}}
	\hspace{0.01cm} 
	\subfloat[\centering\label{TcCfilter_unwrapped}]{\includegraphics[width=.181\linewidth]{TcCfilter_unwrapped}}
	\\
...
\begin{table}[hbt]
	\caption{Efficiency of filters for simulated data\DIFaddbeginFL \DIFaddFL{: mean $\pm$ standard deviation over $R = 30$ independent noise realizations}\DIFaddendFL . \DIFaddbeginFL \DIFaddFL{Runtime (RT) values correspond to the average time among all replications. Time savings are computed relative to Ref.\ Lee and bold entries indicate the best value per column.}\DIFaddendFL \label{TabEficiencia_simu}}
	\DIFdelbeginFL %DIFDELCMD < \begin{tabularx}{\textwidth}{
%DIFDELCMD < 			l % Filter name (left-aligned)
%DIFDELCMD < 			>{\centering\arraybackslash}X  % RMSE
%DIFDELCMD < 			>{\centering\arraybackslash}X  % SSIM
%DIFDELCMD < 			>{\centering\arraybackslash}X  % RT (s)
%DIFDELCMD < 			>{\centering\arraybackslash}X  % Time Saved
%DIFDELCMD < 		}
%DIFDELCMD < 		%%%
\DIFdelendFL \DIFaddbeginFL \begin{tabularx}{\textwidth}{
		...
		\DIFaddendFL \toprule
		\textbf{Filter} & \textbf{RMSE \DIFaddbeginFL \DIFaddFL{(mean $\pm$ sd)}\DIFaddendFL } & \textbf{SSIM \DIFaddbeginFL \DIFaddFL{(mean $\pm$ sd)}\DIFaddendFL } & \textbf{RT (\DIFdelbeginFL \DIFdelFL{s}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\unit{\second}}\DIFaddendFL )}&\textbf{Time Saved (\%)}\\
		\midrule
		Ref. Lee  & \DIFdelbeginFL \DIFdelFL{1.255 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\mathbf{1.84038 \pm 6.8\cdot 10^{-3}}$        }\DIFaddendFL & \DIFdelbeginFL \textbf{\DIFdelFL{0.687}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\mathbf{0.945286 \pm 4.1 \cdot 10^{-4}}$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{4790.76 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{5656.09 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{--- }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\text{---}$}\DIFaddendFL \\
		LInSARRFE & \DIFdelbeginFL \DIFdelFL{1.269 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{${1.84926 \pm 4.7\cdot 10^{-3}}$        }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.644          }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{${0.944787 \pm 2.8\cdot 10^{-4}}$       }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{1460.44 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{624.82 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{69.51 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{88.95 }\DIFaddendFL \\
		TcNfilter & \DIFdelbeginFL \DIFdelFL{1.297 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{${1.879114 \pm 6.6\cdot 10^{-3}}$        }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.670          }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{${0.943092 \pm 3.9\cdot 10^{-4}}$ }\DIFaddendFL & \DIFdelbeginFL \textbf{\DIFdelFL{404.10}}  %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\mathbf{343.78}$  }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{91.56 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\mathbf{93.92}$ }\DIFaddendFL \\
		TcCfilter & \DIFdelbeginFL \textbf{\DIFdelFL{1.251}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{${1.849049 \pm 4.6\cdot 10^{-3}}$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.641 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{${0.944799\pm 2.7\cdot 10^{-4}}$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{427.73  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{433.64  }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{91.07 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{92.33 }\DIFaddendFL \\
		\bottomrule
	\end{tabularx}
\end{table}
...
\begin{figure}[hbt]
	%\begin{adjustwidth}{-\extralength}{0cm}
	\centering
	\subfloat[\centering\label{interferograma_LaCumbe}]{\includegraphics[width=.199\linewidth]{interferograma_LaCumbe}}
	\\
	%\subfloat[ \label{img_medianFilter}]{\includegraphics[width=.179\linewidth]{img_medianFilter}}
	\hspace{0.01cm}
	\subfloat[\centering\label{img_LeeRefined}]{\includegraphics[width=.198\linewidth]{img_LeeRefined}}
	\hspace{0.01cm}
	\subfloat[\centering\label{img_LInSARRFE}]{\includegraphics[width=.199\linewidth]{img_LInSARRFE}}
	\hspace{0.01cm}
	\subfloat[\centering\label{img_TcNfilter}]{\includegraphics[width=.199\linewidth]{img_TcNfilter}}
	\hspace{0.01cm}
	\subfloat[\centering\label{img_TcCfilter}]{\includegraphics[width=.20\linewidth]{img_TcCfilter}}
	\\
	%\vspace{-0.3cm}
	{\includegraphics[width=.67\linewidth]{BarraCores}}
	\\	
	%\subfloat[ \label{unwrap_phase_median}]{\includegraphics[width=.181\linewidth]{unwrap_medianFilter}}
	\hspace{0.01cm}
	\subfloat[\centering\label{unwrap_phase_LeeRefined}]{\includegraphics[width=.199\linewidth]{unwrap_LeeRefined_0705.2}}
	\hspace{0.01cm}
	\subfloat[\centering\label{unwrap_phase_LInSARRFE}]{\includegraphics[width=.201\linewidth]{unwrap_LInSARRFE_0705}}
	\hspace{0.01cm}
	\subfloat[\centering\label{unwrap_phase_TcNfilter}]{\includegraphics[width=.20\linewidth]{unwrap_TcNfilter-1_01}}
	\hspace{0.01cm} 
	\subfloat[\centering\label{unwrap_phase_TcCfilter}]{\includegraphics[width=.201\linewidth]{unwrap_TcCfilter-1-03}}
	\\
	\vspace{-0.3cm}
	\subfloat{\includegraphics[width=.67\linewidth]{BarraCores_unwrap1}}
	%\end{adjustwidth}
	%\caption{\textmd{Interferograms filtered and before unwrapping the phase (top) and after unwrapping (bottom)}}
	\caption{La Cumbre image before and after filtering: (\textbf{a})~Unfiltered Phase, (\textbf{b})~Refined Lee, (\textbf{c})~LInSARRFE, \DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL \textbf{d})~TcNfilter, (\textbf{e})~TcCfilter. Unwrapped phase after filter application: (\textbf{f})~Refined Lee, (\textbf{g})~LInSARRFE, (\textbf{h})~TcNfilter, (\textbf{i})~TcCfilter}
...
\begin{figure}[hbt]
	%\begin{adjustwidth}{-\extralength}{0cm}
	\centering
	\subfloat[\centering\label{InSAR_alamos}]{\includegraphics[width=.20\linewidth]{InSAR_alamos}}
	\\
	%\subfloat[ \label{img_medianFilter}]{\includegraphics[width=.179\linewidth]{img_medianFilter}}
	\hspace{0.01cm}
	\subfloat[\centering\label{img_LeeRefined_alamos}]{\includegraphics[width=.202\linewidth]{img_LeeRefined_alamos}}
	\hspace{0.01cm}
	\subfloat[\centering\label{img_LInSARRFE_alamos}]{\includegraphics[width=.20\linewidth]{img_LInSARRFE_alamos}}
	\hspace{0.01cm}
	\subfloat[\centering\label{img_TcNfilter_alamos}]{\includegraphics[width=.20\linewidth]{img_TcNfilter_alamos}}
	\hspace{0.01cm}
	\subfloat[\centering\label{img_TcCfilter_alamos}]{\includegraphics[width=.20\linewidth]{img_TcCfilter_alamos}}
	\\
	{\includegraphics[width=.67\linewidth]{BarraCores}}
	\\	
	\hspace{0.01cm}
	\subfloat[\centering\label{unwrap_LeeRefined_alamos}]{\includegraphics[width=.2\linewidth]{unwrap_LeeRefined_alamos}}
	\hspace{0.01cm}
	\subfloat[\centering\label{unwrap_LInSARRFE_alamos}]{\includegraphics[width=.201\linewidth]{unwrap_LInSARRFE_alamos}}
	\hspace{0.01cm}
	\subfloat[\centering\label{unwrap_TcNfilter_alamos}]{\includegraphics[width=.20\linewidth]{unwrap_TcNfilter_alamos}}
	\hspace{0.01cm} 
	\subfloat[\centering\label{unwrap_TcCfilter_alamos}]{\includegraphics[width=.201\linewidth]{unwrap_TcCfilter_alamos}}
	\\
	\vspace{-0.3cm}
	\subfloat{\includegraphics[width=.67\linewidth]{BarraCores_unwrap1}}
	%\end{adjustwidth}
	%\caption{\textmd{Interferograms filtered and before unwrapping the phase (top) and after unwrapping (bottom)}}
	\caption{Los Alamos image before and after filtering: (\textbf{a})~Unfiltered Phase, (\textbf{b})~Refined Lee, (\textbf{c})~LInSARRFE, \DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL \textbf{d})~TcNfilter, (\textbf{e})~TcCfilter. Unwrapped phase after filter application: (\textbf{f})~Refined Lee, (\textbf{g	})~LInSARRFE, (\textbf{h})~TcNfilter, (\textbf{i})~TcCfilter}
...
\begin{figure}[hbt]
	%\begin{adjustwidth}{-\extralength}{0cm}
	\centering
	\subfloat[\centering\label{InSAR_ArgVol}]{\includegraphics[width=.20\linewidth]{InSAR_ArgVol}}
	\\
	\hspace{0.01cm}
	\subfloat[\centering\label{img_LeeRefined_ArgVol}]{\includegraphics[width=.20\linewidth]{img_LeeRefined_ArgVol}}
	\hspace{0.01cm}
	\subfloat[\centering\label{img_LInSARRFE_ArgVol}]{\includegraphics[width=.20\linewidth]{img_LInSARRFE_ArgVol}}
	\hspace{0.01cm}
	\subfloat[\centering\label{img_TcNfilter_ArgVol}]{\includegraphics[width=.20\linewidth]{img_TcNfilter_ArgVol}}
	\hspace{0.01cm}
	\subfloat[\centering\label{img_TcCfilter_ArgVol}]{\includegraphics[width=.20\linewidth]{img_TcCfilter_ArgVol}}
	\\
	{\includegraphics[width=.67\linewidth]{BarraCores}}
	\\	
	\hspace{0.01cm}
	\subfloat[\centering\label{unwrap_LeeRefined_ArgVol}]{\includegraphics[width=.20\linewidth]{unwrap_LeeRefined_ArgVol}}
	\hspace{0.01cm}
	\subfloat[\centering\label{unwrap_LInSARRFE_ArgVol}]{\includegraphics[width=.20\linewidth]{unwrap_LInSARRFE_ArgVol}}
	\hspace{0.01cm}
	\subfloat[\centering\label{unwrap_TcNfilter_ArgVol}]{\includegraphics[width=.20\linewidth]{unwrap_TcNfilter_ArgVol}}
	\hspace{0.01cm} 
	\subfloat[\centering\label{unwrap_TcCfilter_ArgVol}]{\includegraphics[width=.20\linewidth]{unwrap_TcCfilter_ArgVol}}
	\\
	\vspace{-0.3cm}
	\subfloat{\includegraphics[width=.67\linewidth]{BarraCores_unwrap1}}
	%\end{adjustwidth}
	%\caption{\textmd{Interferograms filtered and before unwrapping the phase (top) and after unwrapping (bottom)}}
	\caption{Robledo Volcano image before and after filtering: (\textbf{a})~Unfiltered Phase, (\textbf{b})~Refined Lee, (\textbf{c})~LInSARRFE, \DIFaddbeginFL \DIFaddFL{(}\DIFaddendFL \textbf{d})~TcNfilter, (\textbf{e})~TcCfilter. Unwrapped phase after filter application: (\textbf{f})~Refined Lee, (\textbf{g})~LInSARRFE, (\textbf{h})~TcNfilter, (\textbf{i})~TcCfilter}
...
\DIFdelbegin \DIFdel{TcCfilter achieved the lowest RMSE }\DIFdelend \DIFaddbegin \DIFadd{TcNfilter achieved the bigest RT }\DIFaddend values in simulated data, indicating high \DIFdelbegin \DIFdel{precision }\DIFdelend \DIFaddbegin \DIFadd{save time }\DIFaddend in phase estimation\DIFaddbegin \DIFadd{, without compromising accuracy}\DIFaddend . 
...
The \DIFaddbegin \DIFadd{authors highlight that, in this work, we investigated the change from the Lee model to the Gierull physical model and to empirical models, a methodology that demonstrated improvements in computational efficiency.
}
...
%DIFDELCMD < %%%
\DIFdel{The proposed filters’ computational }\DIFdelend \DIFaddbegin \DIFadd{Computational }\DIFaddend efficiency was crucial in the evaluation \DIFaddbegin \DIFadd{of the proposed filters}\DIFaddend .
...
These characteristics are promising given the growing demand for efficient algorithms in missions \DIFdelbegin \DIFdel{like }\DIFdelend \DIFaddbegin \DIFadd{such as }\DIFaddend NISAR, which generate\DIFdelbegin \DIFdel{s}\DIFdelend  massive amounts of interferometric data.
...
This idea can improve numerical integration\DIFdelbegin \DIFdel{and parallel computing}\DIFdelend .
